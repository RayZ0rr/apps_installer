...
install: all
	cp $(TARGET) ~/.local/bin/$(EXE)

static: archive | $(BIN_DIR)
	$(CXX) -o $(TARGET) $(EXE_FLAGS) $(CXX_COMMON) $(CXX_FLAGS) $(MAIN_OBJ) $(LIB_FLAGS) -lapps_installer

archive: $(OBJ) $(MAIN_OBJ) | $(LIB_DIR)
	ar rcv $(LIB_DIR)/libapps_installer.a $(OBJ)

$(TARGET): $(OBJ) $(MAIN_OBJ) | $(BIN_DIR)
	$(CXX) -o $(TARGET) $(EXE_FLAGS) $(CXX_COMMON) $(CXX_FLAGS) -s -DNDEBUG $(LIB_FLAGS) $(OBJ) $(MAIN_OBJ) $(LIBS)
